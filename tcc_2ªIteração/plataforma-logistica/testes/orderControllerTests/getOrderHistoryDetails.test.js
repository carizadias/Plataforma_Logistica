...
describe('GET /api/orders/details/:order_id', () => {
  ...
  it('deve retornar 401 se o token não for fornecido', async () => {
    const res = await request(app).get('/api/orders/details/123');
  ...
  it('deve retornar 404 se o pedido não for encontrado', async () => {
    ...
    const res = await request(app)
      .get('/api/orders/details/123')
      .set('Authorization', `Bearer ${token}`);
    expect(res.status).toBe(404);
    expect(res.body.message).toBe('Pedido não encontrado');
  });
  
  it('deve retornar 200 e os detalhes do pedido se encontrado', async () => {
    const orderMock = {
      order_id: '123',
